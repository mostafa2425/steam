    ...
    if (this.props.history.location.data) {
        ...
        .then((response) => {
          if (response.ok) {
            response.json().then((data) => {
              ...
              this.setState(
                { vendors, Id, StartDate: start, EndDate: end },
                () => {
                  setTimeout(() => {
      ...
      if (!this.props.clubsList.length > 0) {
        ...
        this.setState({ clubs: this.props.clubsList, loading: false }, () => {
          setTimeout(() => {
            // if(forAll){
            //   this.formRef.current.setFieldsValue({ ClubName: "all" });
            // }else{
              this.formRef.current.setFieldsValue({ ClubName: ClubId }); 
            // }
          }, 500);
                ...
                >
                  ...
                  <Form.Item name="rangePicker" label="RangePicker">
                    <RangePicker
                      ...
                      // ]}
                      defaultValue={[
                        moment(this.state.StartDate && this.state.StartDate, 'YYYY/MM/DD'), 
                        moment(this.state.EndDate && this.state.EndDate, 'YYYY/MM/DD'),
                      ]}
                    />
                  </Form.Item>
